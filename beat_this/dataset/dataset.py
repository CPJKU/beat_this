...
class BeatDataModule(pl.LightningDataModule):
    ...
    def get_train_positive_weights(self, widen_target_mask=1, target_kernel_sum=1):
        """
        ...
        `target_kernel_sum` multiplies the number of positive targets by the
        given factor (for convolving the positive targets with a smoothing
        kernel).
        """
        # find the positive weight for the loss as a ratio between (down)beat and non-(down)beat annotation
        dataset = self.train_dataset
        all_frames = sum(dataset.get_frame_count(index)
                         for index in range(len(dataset)))
        all_frames_db = sum(dataset.get_frame_count(index) # consider only datasets which have downbeat information
                         for index in range(len(dataset))
                         if dataset.items[index]["loss_mask"][1])
        any_beat_frames = sum(dataset.get_beat_count(index)
                              for index in range(len(dataset)))
        downbeat_frames = sum(dataset.get_downbeat_count(index) # piece without db infor won't be counted automatcally by get_downbeat_count
                              for index in range(len(dataset)))
        beat_frames = any_beat_frames - downbeat_frames

        return {"beat_and_downbeat" : int(np.round((all_frames - any_beat_frames * widen_target_mask * target_kernel_sum) /
                                                   (any_beat_frames * target_kernel_sum))),
                "beat" : int(np.round((all_frames - beat_frames * widen_target_mask * target_kernel_sum) /
                                      (beat_frames * target_kernel_sum))),
                "downbeat" : int(np.round((all_frames_db - downbeat_frames * widen_target_mask * target_kernel_sum) /
                                          (downbeat_frames * target_kernel_sum))),
                "beat_alpha" : 1 - beat_frames * target_kernel_sum / (all_frames - beat_frames * widen_target_mask * target_kernel_sum),
                "downbeat_alpha" : 1 - downbeat_frames * target_kernel_sum / (all_frames_db - downbeat_frames * widen_target_mask * target_kernel_sum),
                }
